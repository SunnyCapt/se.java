...
interface PointInt extends Updateable {
    double[] get();
}
...
public class Point implements PointInt {
    ...
    @Override
    public void update(double[] _parameters) throws ParametersException {
        if ( _parameters.length != 3  ){
            throw new ParametersException("[EXCEPTION]: Точка задаётся тремя числами");
        }
        x = _parameters[0];
        y = _parameters[1];
        z = _parameters[2];
